...
__Issue:__ The pie chart was a good visual but it did not print the numbers associated with them. There's actually no easy way to do this.  Many outside resources will offer you many confusing ways of doing this.  
  
Custom labels have to be a "factor".  We want to create a label with both the variable name (here it is the categorical data "Female" and "Male") and their count. `paste()` is our friend here.  

And to make it easier, we will convert our Gender table  Again, this is commonly known as "String" in order programming languages.  For me, the easiest way is to create a table using the `table()`. So that we could easily call it

```{r}
data <- table(Titanic$Gender)
data_labels <- paste(names(data),data,sep=", ")
pie(data,labels=data_labels)
```

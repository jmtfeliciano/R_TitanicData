...
For instance, `barplot()` is able to read data from RData seemlessly.  But it is always a challenge to learn the most proper -- and perhaps the simplest -- method to print custom data sets. So my personal struggles with R inspired this tutorials.  
...
For complicated big data, we will merely extract data and plot them.  But for simple small data, we could simply pass an array of values for plotting.   
For some reason, we want to 

```{r}
shipnames <- c("Titanic","Ship_1", "Ship_2")
fatality <- c(.59,.41,.12)
barplot(fatality, main="Fatality Across Different Ships" )
```
